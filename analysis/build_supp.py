...
for i,r in enumerate(items,1):
    fac = "F1 Life Restoration" if r['factor']=='F1' else "F2 Awareness and Management"
...
h.append("<table><tr><th>Domain</th><th class='num'>F1 (Life Restoration)</th><th class='num'>F2 (Awareness &amp; Management)</th><th class='num'>Communality h²</th><th>F1 bootstrap median [95% CI]</th><th>F2 bootstrap median [95% CI]</th><th>Expected</th></tr>")
...
h.append("<table><tr><th>Factor</th><th class='num'>Mean (SD)</th><th class='num'>Median (IQR)</th><th class='num'>Min–Max</th><th class='num'>Skewness</th><th class='num'>Floor / Ceiling</th></tr>"
 "<tr><td>F1 Life Restoration</td><td class='num'>59.5 (14.4)</td><td class='num'>61.5 (51.6–68.2)</td><td class='num'>9.0–86.3</td><td class='num'>−0.62</td><td class='num'>none</td></tr>"
...
rows=[
 ...
 ("Diary fulfilment on headache-free days, 28-day","+0.23 [+0.01, +0.44], 0.043","+0.21 [−0.02, +0.42], 0.070","+0.10 [−0.13, +0.32], 0.384"),
]
h.append("<table><tr><th>External measure</th><th>F1 Life Restoration (prespecified)</th><th>Second factor (exploratory)</th><th>Total (auxiliary)</th></tr>")
for a,b,c,d in rows:
...
h.append("<table><tr><th>Minimum recorded days</th><th class='num'>n</th><th>F1 Life Restoration ↔ headache-day proportion</th><th>Total ↔ headache-day proportion</th></tr>")
for lab,n,f1,tot in dens:
    h.append(f"<tr><td>{lab}</td><td class='num'>{n}</td><td class='num'>{f1}</td><td class='num'>{tot}</td></tr>")
h.append("</table>")
h.append("<p class='note'>Exploratory additional explanatory value (DV = fulfilment on headache-free days, n=75): base model (headache-day proportion + MIDAS) R²=0.01; adding the two factors R²=0.09; the nested-model comparison did not reach significance (F-change(2,70)=2.99, p=0.057). Among the added terms, Life Restoration was the only coefficient reaching nominal significance (standardized β=+0.31, p=0.048); VIF 1.0–1.8; residuals approximately normal (Shapiro p=0.86); maximum Cook's distance 0.16. A percentile bootstrap CI for ΔR² is not reported, because in nested ordinary least squares the in-sample R² cannot decrease and such an interval is biased away from the null.</p>")

h.append("<h2>S3d. Sample-median-split discordance (secondary, descriptive; linked n=81)</h2>")
h.append("<p class='note'>Split at the sample medians of diary headache-day proportion (0.52) and Life Restoration (62.0). Groups are sample-relative and are not clinical thresholds.</p>")
...
h.append("<h2>S3e. Recording behaviour as an alternative explanation for the second factor and for the headache-day–Life Restoration association</h2>")
...
h.append("<table><tr><th>Analysis</th><th>Life Restoration (F1)</th><th>Second factor (F2)</th></tr>"
